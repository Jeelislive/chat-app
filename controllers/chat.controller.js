...
const newGorupChat = TryCatch(async (req, res, next) => {
...
const addMembers = TryCatch(async (req, res, next) => {   
  ...
  if (chat.members.length > 10)
    return next(new ErrorHandler("Group members cannot exceed 10", 400));
  ...
  emitEvent(
    ...
    ALERT,
    chat.members,
    { message: `Users ${allUsersName} have been added to the group`, chatId }
  );
...
const removeMember = TryCatch(async (req, res, next) => {
  ...
  if (chat.members.length <= 3)
    return next(new ErrorHandler("Group members cannot be less than 3", 400));
...
const deleteChat = TryCatch(async (req, res, next) => {
    ...
    //here i have to delete all the messages as well as attachments of the chat from the cloudinary
